...
The following specification outlines this process:
    ...
    fun evaluator(y, []) = false
        | evaluator(capacity, step::stepDown) = (value = capacity) orelse evaluator (capacity , step)
    ...
    #input our N value, and the list of possible denominations
    in coinSplit(N, k, [])
...
For our first iteration we increment up from 2<sup>0</sup> until we reach a value that is in excess of our limit. This would be 2<sup>7</sup> so we decrement by 1 and take that value. 

1. Subproblem 1: N = 100 | 2<sup>6</sup> ---> 64
    Our biggest possible denomination that is below 100. 
...
We will take a different set of tasks:
            ...
            Chosen                             Chosen
    Tasks = (0, 1, 15), (0, 1, 1), (1, 5, 12), (1, 3, 9), (2, 7, 1), 
            
            Chosen    <------>  Chosen                 Missed!
            (2, 8, 7)   Idle    (8, 10, 7), (8, 9, 1), (9, 10, 50)
